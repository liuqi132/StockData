...
from utils.date_util import get_last_days, get_current_day, get_yesterday
...
def get_pre_gkz():
    days = 2
    last_two_days = get_last_days(get_yesterday(), -days)
    pre_stock_dict = {}
    gkz_stock_dict = {}
    ...
    for stock in stock_lists:
        ...
        if (not stock.stock_code.startswith('sz300') and
            ...
            print('{} {} 收盘价:{}, 首日涨幅:{}, 次日涨幅：{}'.format(stock.stock_code, stock.stock_name, stock.price_end, gkz_stock_dict[stock.stock_code],
...
def get_pre_gkz3():
    ...
    last_three_days = get_last_days(get_yesterday(), -days)
    stock_filter = {}
    for stock in stock_lists:
        ...
        for every_day_data in three_days_data:
            if not 1 <= every_day_data.range_increase <= 7 or \
                    every_day_data.price_end > 15 or \
                    every_day_data.price_end < 5:
                break
        ...
        if range_increase_list.__len__() == days and 15 > sum(range_increase_list) > 5:
            stock_filter[stock.stock_name] = range_increase_list
...
def get_pre_gkz4():
    ...
    for stock in stock_lists:
        ...
        range_increase_list = []
        for every_day_data in three_days_data:
            if every_day_data.price_end > 15 or every_day_data.price_end < 2:
                break
            range_increase_list.append(every_day_data.range_increase)
        if range_increase_list.__len__() == days and 10 > sum(range_increase_list) > 0:
            stock_filter[stock.stock_code] = stock.name
    for key, value in stock_filter.items():
        print(key, value)
